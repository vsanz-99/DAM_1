...
public class Agenda {
	...
	// Este constructor intentara abrir el fichero y 
	// si lo consigue volcara todos los datos en el ArrayList.
	...
	public void borrarTelefono(String nombre, String telefono) {
		...
		for (Contacto c: contactos) {
			if (encontrado=c.nombre.equals(nombre))
				index=contactos.indexOf(c);		
		}
		if (encontrado)
			contactos.get(index).borrarTelefono(telefono);
	}
